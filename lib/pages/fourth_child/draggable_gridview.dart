...
class DraggableGridViewDemo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text('draggable 长按拖动'),),
  ...
  Widget _buildDraggable(BuildContext context, int index) {
    return LayoutBuilder(
      builder: (context, constraint) {
        return LongPressDraggable( //index  是 按住的index
          data: index,
          child: DragTarget<int>(
            //松手时 如果onWillAccept返回true 那么久会调用，本案例不使用。
            onAccept: (int data) {
              print(index);  //拖拽的 index  是目标 index
              print(data);   //手指 按的 索引值
            },
            //绘制widget
...
//            _draggingItemIndex = index;
          ...
          feedback: SizedBox(
            ...
            child: widget.itemBuilder(context, _dataList[index]),//手指按的 index
          ),
          //这个是当item被拖动时，item原来位置用来占位的widget，（用户把item拖走后原来的地方该显示啥？就是这个）
          childWhenDragging: Container(
            child: SizedBox(
              child: _showItemWhenCovered
                  ? widget.itemBuilder(context, _dataList[index]) //手指按的index
                  : null,
            ),
          ),
        );
